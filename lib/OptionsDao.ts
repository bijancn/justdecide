...
export async function addOptions(userId: string, topicId: number, optionsText: Array<UserOption>): Promise<Array<Option>> {
...
export async function fetchOptionsOfTopic(topicId: number): Promise<Array<Option>> {
    let { data: result, error } = await supabase
        .from("options")
        .select("*")
        .eq("topic_id", topicId)
